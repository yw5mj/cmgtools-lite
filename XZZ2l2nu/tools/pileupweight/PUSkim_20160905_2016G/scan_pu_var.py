...
pu_list=[60000, 60500, 61000, 61500, 62000, 62500, 63000, 63500, 64000, 64500, 65000, 65500, 66000, 66500, 67000, 67500, 68000, 68500, 69000, 69500, 70000, 70500, 71000, 71500, 72000]
...
#pu_list=[61000, 61500, 62000, 62500, 63000, 63500, 64000, 64500, 65000, 65500, 66000, 66500, 67000, 67500, 68000, 68500, 69000, 69500, 70000, 70500, 71000]

#pu_list=[66500, 66600, 66700, 66800, 66900, 67000, 67100, 67200, 67300, 67400, 67500, 67600, 67700, 67800, 67900, 68000, 68100, 68200, 68300, 68400, 68500, 68600, 68700, 68800, 68900, 69000, 69100, 69200, 69300, 69400, 69500, 69600]

#pu_list=[63000, 63100, 63200, 63300, 63400, 63500, 63600, 63700, 63800, 63900, 64000, 64100, 64200, 64300, 64400, 64500, 64600, 64700, 64800, 64900, 65000, 65100, 65200, 65300, 65400, 65500, 65600, 65700, 65800, 65900, 66000, 66100, 66200, 66300, 66400, 66500, 66600, 66700, 66800, 66900, 67000, 67100, 67200, 67300, 67400, 67500, 67600, 67700, 67800, 67900, 68000, 68100, 68200, 68300, 68400, 68500, 68600, 68700, 68800, 68900, 69000, 69100, 69200, 69300, 69400, 69500, 69600, 69700, 69800, 69900, 70000, 70100, 70200, 70300, 70400, 70500, 70600, 70700, 70800, 70900, 71000]
#pu_list=[69000, 69100, 69200, 69300, 69400, 69500, 69600, 69700, 69800, 69900, 70000, 70100, 70200, 70300, 70400, 70500, 70600, 70700, 70800, 70900, 71000]

#pu_list=[68000, 68100, 68200, 68300, 68400, 68500, 68600, 68700, 68800, 68900, 69000, 69100, 69200, 69300]
#, 69400, 69500, 69600, 69700, 69800, 69900, 70000, 70100, 70200, 70300, 70400, 70500, 70600, 70700, 70800, 70900, 71000]
...
var_wts={'nVert': ["(1)", "(1)"],
          ...
          'nallvtx':["(1)", "(1)"],
          #'corrnvtx':["(1)", "(1)"],
          'corrnvtx':["(1)", "((1.530973+0.697960*nTrueInt)/(1.876572+0.713286*nTrueInt))"],
          #'corrnvtx':["(1)", "((2.47-0.053*nallvtx)*(1-0.000935*nTrueInt-0.00001657*nTrueInt*nTrueInt))"],
          #'corrnvtx':["(1)", "((2.8-0.06*nallvtx)*(1-0.000935*nTrueInt-0.00001657*nTrueInt*nTrueInt))"],
...
ress = array('d', var_ranges[scanvar]*[0.0])
...
for pu in pu_list:
    ...
    hdt = TH1D("hdt","hdt_"+str(pu),var_ranges[scanvar], 0, var_ranges[scanvar])    
    hmc = TH1D("hmc","hmc_"+str(pu),var_ranges[scanvar], 0, var_ranges[scanvar])    
    hdt.SetName('hdt_'+str(pu))
    hmc.SetName('hmc_'+str(pu))
    hdt.SetTitle('{var}: MB xsec= {pu:n} ub {lumitag:s}'.format(var=scanvar,pu=pu,lumitag=lumiTag))
    hdt.Sumw2()
    hmc.Sumw2()
    if DataHLT=="DblLepHLT" :
        dttree.Draw(scanvar+'>>hdt_'+str(pu), '('+cuts+'&&(HLT_MUMU||HLT_ELEL))*'+var_wts[scanvar][0], 'e')
    elif DataHLT=="SglLepHLT" :
        dttree.Draw(scanvar+'>>hdt_'+str(pu), '('+cuts+'&&(HLT_MUv2||HLT_ELEv2))*'+var_wts[scanvar][0], 'e')
    else: 
        dttree.Draw(scanvar+'>>hdt_'+str(pu), '('+cuts+')', 'e')
    mctree.Draw(scanvar+'>>hmc_'+str(pu), '(('+cuts+')*(genWeight*puWeight'+str(pu)+'*'+var_wts[scanvar][1]+'))', 'e')
    hmc.Scale(hdt.Integral()/hmc.Integral())
    hdt.GetXaxis().SetTitle(var_tags[scanvar])
    hmc.GetXaxis().SetTitle(var_tags[scanvar])
    ...
    hres = TH1D("hres_"+str(pu),"hres_"+str(pu), var_ranges[scanvar], 0, var_ranges[scanvar])
    hres.Sumw2()
    ...
    hres.GetXaxis().SetTitle(var_tags[scanvar])
