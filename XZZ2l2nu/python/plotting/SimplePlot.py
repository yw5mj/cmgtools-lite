...
def drawStack_simple(frame, hstack, hdata, hratio, legend,
                     ...
                     lumi=2.169, notes="",
                     blinding=False, blindingCut = 100):
    
    fout = TFile(outDir+'/'+output+'_'+channel+'.root', 'recreate')
    ...
    if blinding:
        for idt in range(hdata.GetNbinsX()+1):
            if hdata.GetBinCenter(idt)>blindingCut:
                hdata.SetBinContent(idt, 0)
                hdata.SetBinError(idt, 0)
            else: pass
        hdata.Draw("Psame")
    else:
        hdata.Draw("Psame")
 ...
 # blinding mask
    if blinding :
        print '[Debug]: I am blinding :D'
        hmask_data = copy.deepcopy(hdata)
        hmask_data.Print("all")
    ...
    if blinding:
        hmask_ratio = copy.deepcopy(hratio)
    ...
    c1.Print(outDir+'/'+output+'_'+channel+'.eps')
