...
fout = ROOT.TFile("uncert_pdf.root", "recreate")
...
#for sample in [sample]: 
    ...
    for idx in wtIds_pdf:
        print ' - PDF '+str(idx)
        allPlotters[sample].changeCorrectionFactor('(LHEweight_wgt['+str(idx)+']/LHEweight_wgt[0])','genWeight')
    ...
    if not ('BulkGrav' in sample):
        print ' - Alpha_s '
        allPlotters[sample].changeCorrectionFactor('(LHEweight_wgt['+str(wtIds_as_bkg[0])+']/LHEweight_wgt[0])','genWeight')
        ...
        allPlotters[sample].changeCorrectionFactor('(LHEweight_wgt['+str(wtIds_as_bkg[1])+']/LHEweight_wgt[0])','genWeight')
