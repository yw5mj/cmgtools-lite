...
fout = ROOT.TFile("uncert_jec_"+channel+".root", "recreate")
...
#for sample in [sample]:
    print 'JEC for '+sample
    hunc_jec[sample] = ROOT.TH1F(sample+'_hunc_jec', sample+'_hunc_jec', 200,0,2)
 
    h = allPlotters[sample].drawTH1(sample+'_mh_jec_wt_0','llnunu_mta',cuts,str(lumi*1000),50,0,5000,titlex = "M_{T}",units = "GeV",drawStyle = "HIST")
    n0 = h.Integral()
    print ' n0 = '+str(n0)
    ...
    cuts_up = cuts.replace('met_pt','met_JECUp_pt')
    h = allPlotters[sample].drawTH1(sample+'_mh_jecup','llnunu_mta',cuts_up,str(lumi*1000),50,0,5000,titlex = "M_{T}",units = "GeV",drawStyle = "HIST") 
    n1 = h.Integral()
    print '  n1 = '+str(n1)+', n1/n0 = '+str(n1/n0)
    hunc_jec[sample].Fill(n1/n0)
    all_var_jec[sample].append(n1/n0)

    cuts_dn = cuts.replace('met_pt','met_JECDown_pt')
    h = allPlotters[sample].drawTH1(sample+'_mh_jecdn','llnunu_mta',cuts_dn,str(lumi*1000),50,0,5000,titlex = "M_{T}",units = "GeV",drawStyle = "HIST")
    n2 = h.Integral()
    print '  n2 = '+str(n2)+', n2/n0 = '+str(n2/n0)
    hunc_jec[sample].Fill(n2/n0)
    all_var_jec[sample].append(n2/n0)

    jec_up = n1/n0
    jec_dn = n2/n0
    jec_unc = abs(jec_up-jec_dn)/2.0
    print '  jec_unc = '+str(jec_unc)+', jec_up = '+str(jec_up)+', jec_dn = '+str(jec_dn)


    hunc_jec[sample].Write()

    all_unc_jec[sample] = jec_unc

print '- all_unc_jec: ',all_unc_jec
print '- all_var_jec: ',all_var_jec
