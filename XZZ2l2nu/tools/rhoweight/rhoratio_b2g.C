{
  ...
  hratio_dt_mc->GetYaxis()->SetRangeUser(0,3);
  ...
  sprintf(name, "f(#rho) = %.2f + %.2f #upoint #rho ", fratio_dt_mc->GetParameter(0), fratio_dt_mc->GetParameter(1));
  lumipt->AddText(0.0,0.6, name);
  lumipt->Draw();

  plots->Print("rho_rewegiht_b2g.pdf");
  plots->SaveAs("rho_rewegiht_b2g.C");


  sprintf(name, "(%.3f+%.3f*x)",
