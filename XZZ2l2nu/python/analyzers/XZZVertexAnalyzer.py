...
class XZZVertexAnalyzer( Analyzer ):
    ...
    def __init__(self, cfg_ana, cfg_comp, looperName):
        super(XZZVertexAnalyzer, self).__init__(cfg_ana, cfg_comp, looperName)
    ...
    def declareHandles(self):
        super(XZZVertexAnalyzer, self).declareHandles()
    ...
    def beginLoop(self, setup):
        super(XZZVertexAnalyzer,self).beginLoop(setup)
    ...
    def process(self,  event):
        ...
        if self.verbose:
            print 'XZZVertexAnalyzer: #vert = ', len(event.vertices), \
...
#           self.pileup.mindist.Fill( self.mindist(event.goodVertices) )
    ...
    def write(self, setup):
        super(XZZVertexAnalyzer, self).write(setup)
        if self.doHists:
            self.pileup.write()

setattr(XZZVertexAnalyzer,"defaultConfig",cfg.Analyzer(
    class_object=XZZVertexAnalyzer,
    vertexWeight = None,
    fixedWeight = 1,
    verbose = False
   )
