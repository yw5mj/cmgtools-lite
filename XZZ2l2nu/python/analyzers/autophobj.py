...
leptonType = NTupleObjectType("lepton", baseObjectTypes = [ particleType ], variables = [
    ...
    NTupleVariable("TuneP_pt",  lambda x : x.physObj.tunePMuonBestTrack().pt() if abs(x.pdgId())==13 and ((hasattr(x,'istag') and x.istag) or (not hasattr(x,'istag'))) else -999, help="TuneP Pt for muon"),
    NTupleVariable("TuneP_ptErr",  lambda x : x.physObj.tunePMuonBestTrack().ptError() if abs(x.pdgId())==13 and ((hasattr(x,'istag') and x.istag) or (not hasattr(x,'istag'))) else -999, help="TuneP Pt error for muon"),
    NTupleVariable("TuneP_eta",  lambda x : x.physObj.tunePMuonBestTrack().eta() if abs(x.pdgId())==13 and ((hasattr(x,'istag') and x.istag) or (not hasattr(x,'istag'))) else -999, help="TuneP Pt for muon"),
    NTupleVariable("TuneP_phi",  lambda x : x.physObj.tunePMuonBestTrack().phi() if abs(x.pdgId())==13 and ((hasattr(x,'istag') and x.istag) or (not hasattr(x,'istag'))) else -999, help="TuneP Pt for muon"),
    NTupleVariable("TuneP_type",  lambda x : x.physObj.tunePMuonBestTrackType() if abs(x.pdgId())==13 and ((hasattr(x,'istag') and x.istag) or (not hasattr(x,'istag'))) else -999, help="TuneP type for muon, https://cmssdt.cern.ch/SDT/doxygen/CMSSW_7_6_3_patch2/doc/html/df/de3/classreco_1_1Muon.html#afceb985a23ee1d456e4dc91391f2e7fe"),
    # Impact parameter
