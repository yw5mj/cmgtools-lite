...
if __name__=='__main__':
    ...
    a=gethist(ag,'Zll_eta',50,-2.5,2.5,'dt')
    ee=geteff(a[1][1],a[0][1],2,'e_data')
    em=geteff(a[3][1],a[2][1],2,'mu_data')
    ee.SetTitle('electron efficiency vs Z_eta;Z_eta;efficiency [%]')
    em.SetTitle('muon efficiency vs Z_eta;Z_eta;efficiency [%]')

    agmc='/afs/cern.ch/work/y/yanchu/graviton/CMSSW_7_6_3_patch2/src/CMGTools/XZZ2l2nu/cfg/trgmc/dy.root' #hadd DY samples
    amc=gethist(agmc,'Zll_eta',50,-2.5,2.5,'dy')
    ...
    azmc=gethist(agzmc,'Zll_eta',50,-2.5,2.5,'zz')
...
#    c.Print('mceledr.png')
    ...
    c.Print('mueta.pdf')
    c.Print('mueta.png')
    esc=TH1F('esc','esc',50,-2.5,2.5)
    esc.SetTitle('electron scale factor vs Z_eta;Z_eta;SF')
    for i in range(1,51):
    ...
    c.Print('esfeta.pdf')
    c.Print('esfeta.png')
    msc=TH1F('msc','msc',50,-2.5,2.5)
    msc.SetTitle('muon scale factor vs Z_eta;Z_eta;SF')
    for i in range(1,51):
