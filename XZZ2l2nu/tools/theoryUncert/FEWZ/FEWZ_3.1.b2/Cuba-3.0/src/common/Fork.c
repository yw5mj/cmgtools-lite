...
#define MASTER(s, ...) \
fprintf(stderr, TERM_RED "master: " s TERM_RESET, __VA_ARGS__)
#define WORKER(s, ...) \
fprintf(stderr, TERM_BLUE "worker: " s TERM_RESET, __VA_ARGS__)
#else
#define MASTER(s, ...)
#define WORKER(s, ...)
#endif
...
static inline void ForkCores(This *t)
{
  ...
  t->ncores = 0; // FEWZ hack to use only one core
#ifdef HAVE_GETLOADAVG
