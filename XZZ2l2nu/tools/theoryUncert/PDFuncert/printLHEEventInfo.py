...
#    print EDMWeightInfo.getWeightIDs(samples[key])
    ...
    evt_id=0
    for event in events:
        event.getByLabel(lheLabel,'','LHE', eventsHandle)
        lheInfo = eventsHandle.product()
        print 'Event: ',evt_id
        evt_id+=1

        # original weight
        print ' Original weight: ',lheInfo.originalXWGTUP()

        # loop all weights
        for wgt in lheInfo.weights():     
            print 'wgt.id: '+str(wgt.id)+', wgt.wgt: '+str(wgt.wgt)
