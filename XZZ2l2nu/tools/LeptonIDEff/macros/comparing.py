...
pogdir='MC_NUM_{0}_DEN_genTracks_PAR_pt_alleta_bin1'.format(sys.argv[2])
names=[' mc',' data',' SF']
pogps=[fpog.Get(pogdir+'/efficienciesMC/histo_pt_MC'),fpog.Get(pogdir+'/efficienciesDATA/histo_pt_DATA'),fpog.Get(pogdir+'/pt_ratio')]
...
for i in range(len(pogps)):
    ...
    pogps[i].SetTitle(sys.argv[2].split('and')[0]+names[i]+';muon_Pt(GeV);')
