...
FormulaEvaluator::FormulaEvaluator( std::string const& iFormula )
{
  ...
  if(info.nextParseIndex != static_cast<int>(iFormula.size()) or info.top.get() == nullptr) {
    ...
    error << "FormulaEvaluatorParseError: " << "While parsing '" << iFormula << "' could not parse beyond '"<<std::string(iFormula.begin(),iFormula.begin()+info.nextParseIndex) <<"'";
...
FormulaEvaluator::throwWrongNumberOfVariables(size_t iSize) const {
  ...
  std::stringstream error;
  error << "WrongNumVariables"<<"FormulaEvaluator expected at least "<<m_nVariables<<" but was passed only "<<iSize;
  throw std::runtime_error(error.str());
}
void 
FormulaEvaluator::throwWrongNumberOfParameters(size_t iSize) const {
  ...
  std::stringstream error;
  error << "WrongNumParameters"<<"FormulaEvaluator expected at least "<<m_nParameters<<" but was passed only "<<iSize;
  throw std::runtime_error(error.str());
}
